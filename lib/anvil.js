...
const DIMENSIONS = {
  overworld: 'region',
  the_nether: path.join('DIM-1', 'region'),
  the_end: path.join('DIM1', 'region'),
};

function regionFolder(worldPath, dimension) {
  return path.join(worldPath, DIMENSIONS[dimension] || DIMENSIONS.overworld);
}

/**
 * Surface grid for an arbitrary block bounding box, loading only the region
 * files it touches. Row-major (z outer, x inner).
 */
function readSurface(worldPath, dimension, minX, minZ, width, depth) {
  ...
  for (let rz = rMinZ; rz <= rMaxZ; rz++) {
    for (let rx = rMinX; rx <= rMaxX; rx++) {
      const region = loadRegionFile(path.join(folder, `r.${rx}.${rz}.mca`));
      if (!region) continue;
...
module.exports = {
  NO_DATA, AIR_NAMES, WATER_NAMES, DIMENSIONS,
  RegionFile, loadRegionFile, clearRegionCache,
  regionFolder, normalizeChunk, analyzeChunk, readSurface,
