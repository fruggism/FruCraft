...
const Atlas = (function () {
  const { state, el, escapeHtml, toast, toLatLng, fromLatLng, roundCoord } = CA;
  ...
  function refreshFeature(layerId, featureId) {
    const layer = CA.findLayer(layerId);
    const feature = CA.findFeature(layerId, featureId);
  ...
  function toggleVertexEditing(featureId, on) {
    ...
    if (on) {
      ...
      const layer = CA.findLayer(entry.layerId);
  ...
  function wireProps(host, feature, layer) {
    ...
    const bindText = (sel, apply) => {
      ...
      if (node) node.addEventListener('input', CA.debounce(() => apply(node.value), 250));
    };
    ...
    for (const key of ['color', 'casingColor', 'fillColor', 'strokeColor']) {
      ...
      if (input) input.addEventListener('input', CA.debounce(() => setStyle(key, input.value), 120));
    }
  ...
  function loadImage(src) {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => reject(new Error(`Tile non caricato: ${src}`));
      img.src = src;
    });
  }

  /** Draw the terrain tiles for `bounds` onto a canvas at `zoom`. */
  async function drawTerrain(ctx, bounds, zoom) {
    ...
    const { worldId } = state.world;
    const dim = state.project.world.dimension;
    ...
    // Load in small batches so a big export doesn't open hundreds of sockets.
    const BATCH = 12;
    for (let i = 0; i < jobs.length; i += BATCH) {
      const slice = jobs.slice(i, i + BATCH);
      const images = await Promise.all(slice.map(({ tx, ty }) => (
        loadImage(`/api/tiles/${worldId}/${dim}/${zoom}/${tx}/${ty}.png`).catch(() => null)
      )));
      images.forEach((img, k) => {
        if (!img) return;
        const { tx, ty } = slice[k];
        const px = (tx * span - bounds.minX) * scale;
        const py = (ty * span - bounds.minZ) * scale;
  ...
  async function exportPNG() {
    ...
    CA.setStatus('export-status', `Composizione immagine ${w}×${h}…`, 'busy');
    ...
    try {
      ...
      await new Promise((resolve) => canvas.toBlob((blob) => {
        CA.download(`${CA.slugify(state.project.name)}.png`, blob);
        resolve();
      }, 'image/png'));
      CA.setStatus('export-status', `PNG esportato (${w}×${h} px)`, 'ok');
    } catch (err) {
      CA.setStatus('export-status', `Export fallito: ${err.message}`, 'err');
    }
  }
  ...
  async function exportSVG() {
    ...
    CA.setStatus('export-status', 'Composizione SVG…', 'busy');
    ...
    CA.download(`${CA.slugify(state.project.name)}.svg`, parts.join('\n'), 'image/svg+xml');
    CA.setStatus('export-status', `SVG esportato (${w}×${h})`, 'ok');
  }
  ...
  function exportGeoJSON() {
    ...
    CA.download(
      `${CA.slugify(state.project.name)}.geojson`,
      JSON.stringify({ type: 'FeatureCollection', features }, null, 2),
      'application/geo+json'
    );
    CA.setStatus('export-status', `GeoJSON esportato (${features.length} elementi)`, 'ok');
  }
